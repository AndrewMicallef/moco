...
def moco(stack, template, max_trans = 0, ds_factor = 0):
	...
	frames, height, width = stack.shape
		...
		#Create a stack of template images, downsampling if necessary
        // Template processing
	 ...
	 public static ShortProcessor applyAffine(int[] xy, ShortProcessor oldImage, int rows, int cols)
	 {
		 ...
		 if (xy[0] >= 0 && xy[1] >= 0)
		 {
			 for (int r = xy[1]; r < rows; r++)
			 {
				 ...
				 {
					 // newImage.set(r + rows*c, oldImage.get(r-xy[1] + rows*(c-xy[0])));
				 ...
				 for (int i=-1; i<=1; i++)
				 {
					 for (int j=-1; j<=1; j++)
					 {
						 ...
						 for (int r = Lx-1; r<Rx; r++)
						 {
							 ...
							 {
								 // z[i+1][j+1] = z[i+1][j+1] + (a.get(r + c*rows) - t.get(r+x + rows*(c+y)))*(a.get(r + c*rows) - t.get(r+x + rows*(c+y)));
